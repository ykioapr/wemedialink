 ...
 * @LastEditTime: 2024-10-12 19:34:49
...
_win.bd.on('shown.bs.tab', 'a[data-ajax]', function () {
    $(this).closest('.tab-nav-theme,.tab-nav-but,.index-tab>ul').Tabcenter();
});
...
$.fn.sidebarAffix = function () {
    ...
    var sidebarAffix_con = _this.children('.sidebar-affix');
    ...
    if (_this.children('.sidebar-affix').length) {
        sidebarAffix_con.data('bs.affix').options.offset = get_offset();
        return _this;
    }

    var offset = get_offset();

    sidebarAffix_con = $('<div class="sidebar-affix affix-top" style="width:' + affix_width + 'px;"></div>');
...
$.fn.autoAffix = function (top, bottom, auto_m) {
    ...
    _this
        ...
        .on('affix.bs.affix', function () {
            ...
            if (auto_m) {
                $(this).after($(this).clone().removeClass('affix-top').addClass('affix-clone').css('opacity', '0'));
            }
            _this.css(set_css);
        });
