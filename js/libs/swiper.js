 ...
 * @LastEditTime: 2024-05-20 11:44:32
 * 修改部分内容：
 * 1.删除map
 * 2. translate3d(0,0,0) scale( => translate(0,0) scale(
 * 3. 新增鼠标滚轮 speed 切换时间参数  mousewheel_speed = this.params.mousewheel.speed
 */
...
(function (global, factory) {
  ...
  var Mousewheel = {
    ...
    animateSlider: function animateSlider(newEvent) {
      ...
      //zibll : 修改内容：新增speed 切换时间参数
      var mousewheel_speed = this.params.mousewheel.speed || void 0;
      if (newEvent.direction < 0) {
        if ((!swiper.isEnd || swiper.params.loop) && !swiper.animating) {
          swiper.slideNext(mousewheel_speed);
          swiper.emit('scroll', newEvent.raw);
        }
      } else if ((!swiper.isBeginning || swiper.params.loop) && !swiper.animating) {
        swiper.slidePrev(mousewheel_speed);
        swiper.emit('scroll', newEvent.raw);
